...
const roadGraph = buildGraph(roads);  //console.log(roadGraph); //this is object with each .place as property with value = [] places it touches

const VillageState = class VillageState {
    constructor(place, parcels) {         //simply the constructor which gets called at the end of VillageState.random initially
...
function goalOrientedRobot({ place, parcels }, route) {   //in runRobot function, robot (state, memory), {place, parcels} = state, route = memory
    if (route.length === 0) {
        ...
        if (parcel.place !== place) {      //if the parcel was not just picked up at the current place
            ...
            route = findRoute(roadGraph, place, parcel.address);  // if the parcel's current place is robots place, findRoute with parcel's address (where it's going)
        }
    } //console.log(route);
